...
└── MasterLayout
    ├── ConsentBanner
    ├── Header
    ├── TimeStrip  (mobile-only Sunday time bar)
    ├── Router
    │   ├── Main (Home) — verse card, welcome, mission, congregation photo, outreach, CTA band, SideBar
    │   ├── Location
    │   ├── About
    │   └── ScriptureStudy
    └── Footer
...
Note: the header photo and the fixed sidebar were removed in the Living Word restyle (2026-09). The SideBar renders inside the Home page only.
...
**Rendered by**: HomePage (last section). Reads times from `src/config/events.ts`.

#### Component Structure
See the file; times are not hard-coded here.
...
### TimeStrip.tsx
**Type**: Function Component  
**Purpose**: Thin bar under the header showing the Sunday service time on small screens  
**Location**: `src/components/layout/TimeStrip/TimeStrip.tsx`  
**Data**: `EVENTS[0].time` from `src/config/events.ts`
...
### Image Assets
- **Congregation Photo**: `public/images/congregation.jpg` (1424x640 JPEG, <=200 KB). Shown on Home after "Our Mission" and on About at the foot of "Our History". Replace only under a new filename (`congregation-v2.jpg`) — see the maintainer guide.
- **Icons**: SVGs under `src/assets/svg/` (Cross, Bible, CommunityService, Leadership, Church, HistoryScroll, Timeline). No emoji in page files.
- **Share card**: `public/share-card.png` (1200x630, text-only; regenerated by `scripts/share-card/render.mjs`).
